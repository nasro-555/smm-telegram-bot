...
import { WHATSAPP_PANELS } from "./whatsapp.js";
import { KICK_PANELS } from "./kick.js";
import { THREADS_PANELS } from "./threads.js";
import { LINKEDIN_PANELS } from "./linkedin.js";
import { GOOGLE_MAPS_PANELS } from "./googlemaps.js";
import { LIKEE_PANELS } from "./likee.js";
import { SNAPCHAT_PANELS } from "./snapchat.js";
import {
  getAllServices,
  createSmmxOrder,
  getSmmxOrderStatus,
...
function sellingRateFor(panel, providerRate) {
  const mode = panel.pricing?.mode;
  const value = Number(panel.pricing?.value ?? 0);

  if (mode === "fixed") return value;
  if (mode === "multiply") return providerRate * value;
...
function normalizeService(item, panel) {
  ...
  const sellingRate = Number(
    sellingRateFor(panel, providerRate).toFixed(4)
  );
  ...
  const configuredName = panel.serviceLabels?.[serviceId];
  ...
  return {
    ...
    panelCode: panel.code,
    service: serviceId,
    name: String(configuredName ?? item.name ?? `Service ${item.service}`),
    type,
...
export async function requestRefill(orderId) {
  return requestSmmxRefill(orderId);
}

export async function getRefillStatus(refillId) {
  return getSmmxRefillStatus(refillId);
}

export async function requestCancel(orderId) {
  return requestSmmxCancel(orderId);
}
